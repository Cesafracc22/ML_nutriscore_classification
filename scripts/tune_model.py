...
def main():
    ...
    parser.add_argument('--gpu', action='store_true', help='Use CUDA GPU for XGBoost (implies n_jobs=1 to avoid GPU contention)')
    ...
    if args.gpu and args.model != 'xgboost':
        print("Warning: --gpu is only supported for xgboost; ignoring.")
    if args.gpu and args.model == 'xgboost':
        args.n_jobs = 1
        print("Using GPU (device=cuda); n_jobs set to 1 to avoid GPU contention.")

    estimator = ModelRegistry.create_model(args.model)._build_model()
    if args.gpu and args.model == 'xgboost':
        estimator.set_params(device='cuda')
